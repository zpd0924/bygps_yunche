//
//  BYEditorCarInfoViewController.m
//  BYGPS
//
...
@interface BYEditorCarInfoViewController ()<UITableViewDelegate,UITableViewDataSource,VinManagerDelegate,PlateCameraDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate>
...
@implementation BYEditorCarInfoViewController
...
- (void)initBase{
    [self.navigationView setTitle:@"编辑车辆信息"];
    //    self.model.brand = @"其他";
...
-(void)initDataBase{
    ...
    self.model.carNum = self.carModel.carNum;
    self.model.carFirstNum = self.carModel.carNum.length > 0 ? [self.carModel.carNum substringWithRange:NSMakeRange(0, 2)] : @"粤B";
    self.model.carLastNum = self.carModel.carNum.length > 0 ? [self.carModel.carNum substringFromIndex:2] : @"";
    self.model.carVin = self.carModel.carVin;
    self.model.groupName = self.carModel.groupName;
    self.model.company = self.carModel.groupName;
    self.model.companyId = self.carModel.carGroupId;
    self.model.brand = self.carModel.brand;
    self.model.carType = self.carModel.carType;
    self.model.carModel = self.carModel.carModel;
    self.model.color = self.carModel.carColor;
    self.model.ownerName = self.carModel.ownerName;
    self.model.ownerTel = self.carModel.ownerTel;
    self.model.carId = self.carModel.carId;
    self.model.contractNo = self.carModel.contractNo;
...
//    dict[@"color"] = self.model.color;
    ...
    dict[@"id"] = @(self.model.carId);
    
    [BYSendWorkHttpTool POSTEditCarParams:dict success:^(id data) {
        dispatch_async(dispatch_get_main_queue(), ^{
            BYShowSuccess(@"增加成功");
            self.carModel.carNum = self.model.carNum;
            self.carModel.carVin = self.model.carVin;
            self.carModel.groupName = self.model.company;
            self.carModel.brand = self.model.brand;
            self.carModel.carType = self.model.carType;
            self.carModel.carModel = self.model.carModel;
            self.carModel.carColor = self.model.color;
            self.carModel.ownerName = self.model.ownerName;
            self.carModel.ownerTel = self.model.ownerTel;
            self.carModel.carId = self.model.carId;
            self.carModel.carGroupId = self.model.companyId;
            self.carModel.contractNo = self.model.contractNo;
            if (weakSelf.model.carNum.length <= 0) {
                if (weakSelf.addCarInfoViewBlock) {
                    weakSelf.addCarInfoViewBlock(self.carModel);
                }
            }else{
                if (weakSelf.addCarInfoViewBlock) {
                    weakSelf.addCarInfoViewBlock(self.carModel);
