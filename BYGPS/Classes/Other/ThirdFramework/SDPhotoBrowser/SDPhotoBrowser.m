...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, MAXHEIGHT - 60, 80, 30);
...
{
    //防止数组越界
    if (index >= _scrollView.subviews.count || _scrollView == nil) {
        return [BYProgressHUD by_showErrorWithStatus:@"浏览图片出现问题"];
    }
...
- (void)layoutSubviews
{
    ...
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, MAXHEIGHT - 60);
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowser:showImageIndex:)]) {
         [self.delegate photoBrowser:self showImageIndex:index];
    }
